...
void print_vec_i8(const int8_t *outv, std::string outn, int start, int end, int row_size, float o_amax) {
  std::cout << outn << ": ";
  for(int i=start; i<end; i++) {
    std::cout << static_cast<float>(outv[i])/127.0f*o_amax << " ";
    if((i-start+1)%row_size==0) std::cout << std::endl;  
  }
  std::cout << std::endl;
}

void compareResultsWithGoldenI8(const int8_t *res, const float *ref, int len, float o_amax) {
  ...
  for (unsigned int i_row = 0; i_row < len/64; i_row++) {
    ...
    for (unsigned int i_col = 0; i_col < 64; i_col++) {
      ...
      float res_f = static_cast<float>(res[i])/127.0f*o_amax;
      float ref_f = ref[i];
    ...
    if(this_row_deviation>20 && false) {
      ...
      print_vec_i8(res, "h_mat_o: ", i_row*64, (i_row+1)*64, 64, o_amax);
